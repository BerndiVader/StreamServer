...
public final class Broadcast {
	...
	public static Optional<LiveBroadcastPacket>currentLiveBroadcast=Optional.ofNullable(null);
	public static Optional<LiveStreamPacket>currentLiveStream=Optional.ofNullable(null);
	...
	public static Future<Packet>getLiveBroadcast(BroadcastStatus broadcastStatus) {
		return Helper.EXECUTOR.submit(()->{
			...
			String url=Youtube.URL.concat("liveBroadcasts?part=id,snippet,contentDetails,monetizationDetails,status&broadcastStatus="+status.name()+"&key=").concat(Config.YOUTUBE_KEY);
				...
				return Youtube.HTTP_CLIENT.execute(get,response->{
					...
					if(json==null||json.isJsonNull()) {
						...
						if(json.has("items")&&!json.get("items").getAsJsonArray().isEmpty()) {
							LiveBroadcastPacket broadcast=(LiveBroadcastPacket)Packet.build(json.getAsJsonArray("items").get(0).getAsJsonObject(),LiveBroadcastPacket.class);
							currentLiveBroadcast=Optional.of(broadcast);
							return broadcast;
						} else {
	...
	public static Future<Packet>insertLiveBroadcast(String title,String description,PrivacyStatus privacy) {
		return Helper.EXECUTOR.submit(()->{
			...
			String url=Youtube.URL.concat("liveBroadcasts?part=id,snippet,contentDetails,status&key=").concat(Config.YOUTUBE_KEY);
	...
	public static Future<Packet>inserttLivestream(String title,String description,String privacy) {
		...
		return Helper.EXECUTOR.submit(()->{
			...
			String url=Youtube.URL.concat("liveStreams?part=snippet,cdn,contentDetails,status&key=").concat(Config.YOUTUBE_KEY);
	...
	public static Future<Packet> bindBroadcastToStream(String broadcastId,String streamId) {
		return Helper.EXECUTOR.submit(()->{
			...
			String url=Youtube.URL.concat("liveBroadcasts/bind?id="+broadcastId+"&streamId="+streamId+"&part=id,snippet,contentDetails,status&key=").concat(Config.YOUTUBE_KEY);
			HttpPost post=new HttpPost(url);
			post.setHeader("Authorization","Bearer ".concat(Config.YOUTUBE_ACCESS_TOKEN));
			post.addHeader("Accept","application/json");
			post.addHeader("Content-Type","application/json");			
	...
	public static Future<Packet> transitionBroadcastStatus(String broadcastId,String status) {
		return Helper.EXECUTOR.submit(()->{
			...
			String url=Youtube.URL.concat("liveBroadcasts/transition?broadcastStatus="+status+"&id="+broadcastId+"&part=id,snippet,contentDetails,status&key=").concat(Config.YOUTUBE_KEY);
			HttpPost post=new HttpPost(url);
			post.setHeader("Authorization","Bearer ".concat(Config.YOUTUBE_ACCESS_TOKEN));
			post.addHeader("Accept","application/json");
			post.addHeader("Content-Type","application/json");
	...
	public static Future<Packet> getDefaultLiveStream() {
		return Helper.EXECUTOR.submit(()->{
			...
			String url=Youtube.URL.concat("liveStreams?part=id,snippet,cdn,status&mine=true&key=").concat(Config.YOUTUBE_KEY);
			HttpGet get=new HttpGet(url);
			get.setHeader("Authorization","Bearer ".concat(Config.YOUTUBE_ACCESS_TOKEN));
			get.addHeader("Accept","application/json");
					...
					if(json==null||json.isJsonNull()) {
						...
						if(json.has("items")&&!json.getAsJsonArray("items").isEmpty()) {
							...
							for(JsonElement element:array) {
								...
								if(packet.cdn.ingestionInfo.streamName.equals(Config.STREAM_KEY)) {
									currentLiveStream=Optional.of(packet);
									return packet;
								}
							}
						}
