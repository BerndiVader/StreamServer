...
public final class Server {
				...
				try (InputStream input = exchange.getRequestBody()) {
					...
					if (packet == null) {
						...
						case "publish":
							if (packet.query.equals(Config.LIVESTREAM.SECRET)
									&& packet.protocol.equals(Config.LIVESTREAM.PROTOCOL) && packet.path
											.equals(Config.LIVESTREAM.PATH + "/" + Config.LIVESTREAM.STREAM_KEY)) {
						...
						case "read":
							if (packet.protocol.equals(Config.LIVESTREAM.PROTOCOL)
									...
									&& (Config.LIVESTREAM.WATCHER_SECRET.isEmpty()
											|| Config.LIVESTREAM.WATCHER_SECRET.equals(packet.query))) {

								if (Live.isLive(packet.token)) {
									ANSI.info(String.format("Watcher accepted from %s with ID: %s", packet.ip,
											packet.id));
					...
					if (code != CODE.OK) {
						if (packet != null) {
							...
							ANSI.error("Auth connection rejected: invalid or null packet",
									new Exception("StreamPacket is NULL"));
						}
					}
	...
	private static void offline(HttpExchange exchange) {
		...
		String json = Live
				.query2jsonString(URLDecoder.decode(exchange.getRequestURI().getQuery(), StandardCharsets.UTF_8));
		StatusPacket packet = Helper.GSON.fromJson(json, StatusPacket.class);
